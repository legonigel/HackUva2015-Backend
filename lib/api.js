/**
 * Core api functions. Handle requests exposed by the public application api. The API module is invoked any time a request
 ...
 *
 * @param messageId = [Integer]		id of conversation item to fetch (Does NOT include messages)
 * @param callback 	= [Function]	function to call after message is returned
 */
	...
	if(apiRequestFragment[0] == 'message') {
		...
		API.getMessageFromDatabase(apiRequestFragment[1], function(err, rows, cols) {
			...
			if(err) {
				...
				return response.end('Mysql error: ' + err);
			}
		...
		if(apiRequestFragment[1] == 'conversation') {
			...
			if(!apiRequestFragment[2]) {
				return response.end('Api error: A conversation id is required');
			}
			...
			response.end('unimplemented api');
		}
		...
		if(apiRequestFragment[1] == 'user') {
			...
			if(!apiRequestFragment[2]) {
				return response.end('Api error: A user id is required');
			}
		...
		console.log('Unimplemented api request: ' + apiRequest);
		response.end('Unimplemented api request: ' + apiRequest);
	}
